...
import { sendResponseToOperators, sendErrorNotification } from '@/lib/slack/client';
import { generateEmbedding, findSimilarDocuments, generateChannelioResponse } from '@/lib/ai/openai';
...
export async function POST(request: NextRequest) {
  ...
  console.log(`[${new Date().toISOString()}] [${requestId}] Received POST request for: ${request.url}`);
  ...
  try {
    ...
    console.log(`[${requestId}] 問い合わせ: "${inquiry.substring(0, 100)}${inquiry.length > 100 ? '...' : ''}"`);
    console.log(`[${requestId}] ユーザー: "${userName}", チャットID: "${chatId}"`);
    ...
    try {
      ...
      console.log(`[${requestId}] 埋め込みベクトル生成完了 (次元数: ${embedding.length}, 所要時間: ${Date.now() - startEmbedding}ms)`);
      
      // 類似ドキュメントを検索
      console.log(`[${requestId}] 類似ドキュメントを検索中...`);
      const startSearch = Date.now();
      const similarDocuments = await findSimilarDocuments(embedding);
      console.log(`[${requestId}] 類似ドキュメント検索完了 (件数: ${similarDocuments.length}, 所要時間: ${Date.now() - startSearch}ms)`);
      
      if (similarDocuments.length === 0) {
        console.warn(`[${requestId}] 類似ドキュメントが見つかりませんでした。ダミーデータを使用します。`);
      }
      ...
      // 新しい関数を使用して回答を生成
      const responseDraft = await generateChannelioResponse(inquiry, similarDocuments, customerInfo);
      console.log(`[${requestId}] AI回答案生成完了 (所要時間: ${Date.now() - startGeneration}ms)`);
      ...
      console.log(`[${requestId}] Slack通知送信完了 (所要時間: ${Date.now() - startSlack}ms)`);
      
      // 全処理時間を記録
      const totalProcessingTime = Date.now() - startEmbedding;
      console.log(`[${requestId}] 全処理完了 (合計所要時間: ${totalProcessingTime}ms)`);
      ...
      try {
        ...
        console.error(`[${requestId}] フォールバック処理も失敗:`, fallbackError);
        return handleApiError(processingError, `channelio-webhook-processing-${requestId}`);
      }
    }
  } catch (error) {
...
export async function GET(request: NextRequest) {
  const requestId = `req_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  console.log(`[${new Date().toISOString()}] [${requestId}] Received GET request: ${request.url}`);
  ...
  try {
    ...
    console.error(`[${requestId}] GETテストのSlack通知の送信に失敗:`, slackError);
  }
